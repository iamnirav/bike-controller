...
MUTANTS = [
    ...
    ("bike_controller/mapping.py", "a latched console keeps holding sprint",
     "            out.sprint = False\n            out.at_max = False",
     "            pass"),
    ("bike_controller/mapping.py", "a latched console keeps holding threshold buttons",
     "        if frozen:\n",
     "        if False:\n"),
    ("bike_controller/mapping.py", "a latch floors movement again",
     "        stale = self.tracker.is_stale(now)\n",
     "        stale = self.tracker.is_stale(now) or frozen\n"),
    ("bike_controller/mapping.py", "blackout grace never expires",
     "                    and now - self._blackout_since < movement.blackout_grace):",
     "                    and True):"),
    ("bike_controller/mapping.py", "blackout grace holds a scale never measured",
     "                    and self._last_live_scale is not None\n",
     ""),
    ("bike_controller/mapping.py", "blackout grace latches sprint on no evidence",
     "                self._sprinting = False\n                return self._last_live_scale, False, self._at_max",
     "                return self._last_live_scale, self._sprinting, self._at_max"),
    ("bike_controller/mapping.py", "resumed telemetry does not end the grace",
